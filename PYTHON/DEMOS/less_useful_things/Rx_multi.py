#!/usr/bin/python
"""
 SISO_RX.py -based RX_all_boards.py
...
#    timesRx = np.zeros((numSdrs, 
    ...
    for i, sdr in enumerate(sdrs):
        ...
        a, b, peaks1 = find_lts(sampsRx[i][1], thresh=lts_thresh)

        print("LTS Peaks ({0}): {1}, {2}", i, peaks0, peaks1)

    # If recording samples
    if recorder is not None: 
        frame = sampsRx
        #frame = np.empty((numSdrs, 2, sampsRx[0][0].size), dtype='complex64')
        #for i in range(numSdrs):
        #    frame[i] = sampsRx[i]
        recorder.save_frame(frame, sr.timeNs) # sr.timeNs needs to be given its own array for all boards
#        recorder.save_frame(frame)
    ...
    # Magnitude of IQ Samples (this won't work right, copied from SISO)
    I = np.real(sampsRx[0])
    Q = np.imag(sampsRx[0])
    ...
    # Compute Power of Time Domain Signal (won't work right, SISO)
    ...
    # Compute Power of Frequency Domain Signal (FFT) (won't work right)
    ...
    # Retrieve RSSI computed in the FPGA (again...)
    ...
    line1.set_data(range(sampsRx[0][0].size), np.real(sampsRx[0]))
    line2.set_data(range(sampsRx[0][0].size), np.imag(sampsRx[0]))
    line3.set_data(range(sampsRx[0][0].size), np.abs(sampsRx[0]))
    line4.set_data(range(sampsRx[0][0].size), np.abs(sampsRx[1]))
    line5.set_data(range(sampsRx[0][0].size), np.angle(sampsRx[0]))
    line6.set_data(range(sampsRx[0][0].size), np.angle(sampsRx[1]))
    line7.set_data(range(sampsRx[0][0].size), np.angle(sampsRx[0] * np.conj(sampsRx[1])))
    ...
    line14.set_data(range(sampsRx[0][0].size), np.real(peaks0[:sampsRx[0][0].size]))

    return line1, line2, line3, line4, line5, line6, line7, line8, line9, line10, line11, line12, line13, line14
    '''

# won't work right, sampsRx buffer wrong dimensions
...
def main():
    parser = OptionParser()
    parser.add_option("--label", type="string", dest="label", help="label for recorded file name", default="TEST")
    ...
    parser.add_option("--serials", type="string", dest="serials", help="Serial numbers of the devices", default="./bs_serials.txt")
