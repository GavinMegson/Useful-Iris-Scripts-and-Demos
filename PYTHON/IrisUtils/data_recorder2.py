...
class DataRecorder:
    ...
    def init_datasets(self, group=None):
        ...
        self.samples = group.create_dataset("Samples", (self.num_frame, self.numBoards, 2, self.numSamps),
                dtype=np.complex64, maxshape=(None, self.numBoards, 2, self.numSamps), chunks=(self.num_frame, self.numBoards, 2, self.numSamps))
        self.timeref = group.create_dataset("Time", (self.num_frame, 2),
                dtype=np.uint64, maxshape=(None, 2), chunks=(self.num_frame, 2))
        group['Samples'].attrs['column_names'] = [a.encode('utf8') for a in ['Frame', 'Board', 'Antenna', 'Samples'] ]
