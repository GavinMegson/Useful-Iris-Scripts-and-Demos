...
def init(bnodes, cnodes, ref_ant, ampl, rate, freq, txgain, rxgain, cp, \
    ...
    # base station CSI
    bs_csi(iterations, bsdrs, pilot, cp, prefix_length, postfix_length, use_trig)
...
def cl_csi(iterations, bsdrs, csdrs, pilot, cp, prefix_length, postfix_length, wait_trigger):
    ...
    if wait_trigger:
        bsTxFlags |= SOAPY_SDR_WAIT_TRIGGER
...
def bs_csi(iterations, bsdrs, pilot, cp, prefix_length, postfix_length, wait_trigger):
    ...
    # final signal
    pilotPadded = np.concatenate([pad1, pilot, pad2]).astype(np.complex64)
    pilotZeros = np.zeros(pilotPadded.shape, np.complex64)

    print("Pilot Padded/Zeros shape:", pilotPadded.shape)
 ...
 #       replay_addr = 0 # sometimes used to add multiple signals into one RAM block
  #      sdr.writeRegisters("TX_RAM_A", replay_addr, cfloat2uint32(pilotPadded, order='QI').tolist())
   #     sdr.writeRegisters("TX_RAM_B", replay_addr, cfloat2uint32(pilotPadded, order='QI').tolist())
    ...
    if wait_trigger:
        txFlags |= SOAPY_SDR_WAIT_TRIGGER
    ...
    for iter in range(iterations):
        ...
        # activate tx streams
        print("Activating streams")
        for j, sdr in enumerate(bsdrs):
                rc = sdr.activateStream(txStreams[j])
                if rc < 0:
                    print("problem activating tx stream", j)

        # for each antenna, send pilot
        for j, ref_sdr in enumerate(bsdrs):
            for ref_ch in [0, 1]:
                # specify the transmitting (reference) antenna
                ref_ant = (j, ref_ch)
...
#                            rtrncd = sdr.readStream(rxStreams[i][ch], [bsRxSamps[i][ch]], numSamps, rxFlags)
        ...
        for j, sdr in enumerate(bsdrs):
            print("deactivating streams")
            sdr.deactivateStream(txStreams[j])
