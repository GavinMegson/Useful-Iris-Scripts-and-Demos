...
class DataRecorder:
    ...
    def __init__(self, tag, serial, freq, lna, pga, tia, lna1, lna2, attn, numSamps, numBoards, latitude,longitude,elevation):
        self.num_frame = 1
        self.frame_number = 0
        ...
        self.inputs = self.numBoards*2
        self.outputs = self.numBoards*2
    ...
    def init_h5file(self, filename=None, init_datasets=True):
        ...
        if init_datasets:
            ...
            self.init_datasets()
            self.h5file.attrs['serials'] = self.serial
    ...
    def init_datasets(self, group=None):
        ...
        self.samples = group.create_dataset("Samples", (self.num_frame, self.inputs, self.outputs, self.numSamps),
                dtype=np.complex64, maxshape=(None, self.numBoards*2, self.numBoards*2, self.numSamps), chunks=(self.num_frame, self.inputs, self.outputs, self.numSamps))
        self.timeref = group.create_dataset("Time", (self.num_frame, 2),
                dtype=np.uint64, maxshape=(None, 2), chunks=(self.num_frame, 2))
        group['Samples'].attrs['column_names'] = [a.encode('utf8') for a in ['Frame', 'TxAntenna', 'RxAntenna', 'Samples'] ]
