...
def rxsamples_app(srl, freq, gain, num_samps, recorder, agc_en, wait_trigger):
    ...
    # Global declarations
    global sdrs, rxStreams, freqScale, Rate

    # Instantiate device
#    for i, sdr in enumerate(serials):
#        sdrs[i] = SoapySDR.Device(dict(serial=srl))
...
def main():
    ...
    if options.rxMode == "REC":
        filename = "./data_out/rx" + '%1.3f' % (float(options.freq)/1e9) + 'GHz_' + options.label + '_' + options.serial + '.hdf5'
        recorder = DataRecorder(tag = options.label,
                                ...
                                tia = -1, # using -1 to indicate not used/specified
                                pga = -1,
                                lna = -1,
                                lna1 = -1,
                                lna2 = -1,
