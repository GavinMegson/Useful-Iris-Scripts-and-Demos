...
#    med_nonzero_CSI = np.ma.median(masked_CSI, axis=(2))
    med_nonzero_CSI = np.ma.median(np.ma.masked_array.get_real(masked_CSI),axis=(2)) + 1j*np.ma.median(np.ma.masked_array.get_imag(masked_CSI), axis=(2))
...
#    med_nonzero_noise = np.ma.median(np.ma.get_real(masked_noise),axis=(2)) + 1j*np.ma.median(np.ma.get_imag(masked_noise), axis=(2))
...
################################
    ...
    # For simplicity, assume the message signal to send is all 1s
...
#    U, E, V_H = np.linalg.svd(rel_CSI,full_matrices=False)
    ...
    # last D_tx columns of V
    soft_weights = np.zeros((M_tx,D_tx),dtype=np.complex64)

    # get columns of V, aka conjugated rows of V_H
    soft_conj = np.transpose(np.conjugate(soft_weights))
    for i in range(M_tx-D_tx, M_tx):
        soft_conj[i-(M_tx-D_tx)] = V_H[i]
    soft_weights = np.transpose(np.conjugate(soft_conj))
#    print("DETERMINANT:",np.linalg.det(P_soft))


    print("P_soft:\n", P_soft)
    print("scale of P soft:",abs(P_soft))
    print("angle of P soft:",np.angle(P_soft))

    # normalized softnull
    P_soft_norm = P_soft / np.linalg.norm(P_soft)
    print("normalized P_sfot:",P_soft_norm)
    print("scale:",abs(P_soft_norm))
    print("angle:",np.angle(P_soft_norm))
    



    ######
    # softnul hybrid approximation

    # max and min values for antenna power readings
    max_range = np.amax(abs(P_soft))
#    max_range = 2*np.mean(abs(P_soft))
    min_range = 0

    # list of powres of 2 for granularity to try
    powers = list(range(1,25))

    # calculate the approximation of softnull
    def halfify(top_range, bot_range, target, power):
        top=top_range
        bot=bot_range
        dif = top - bot
        topdif = top - target
    ...
    # calculate approximation of phase
    def phasify(target, power):
        step = 2*np.pi/(2**power)
        targ = target
        ticker = step
        while(ticker < 2*np.pi):
            if abs(targ-ticker) <= step/2:
                return ticker
            ticker += step
        return 0

    # save approximations of P_down
    soft_approxes = np.zeros((len(powers),M_tx,M_rx),dtype=np.complex64)
    # normalized
    norm_soft_approxes = np.zeros((len(powers),M_tx,M_rx),dtype=np.complex64)
    # what happens when only phase is shifted
    phase_approxes = np.zeros((len(powers),M_tx,M_rx),dtype=np.complex64)
    norm_phase_approxes = np.zeros((len(powers),M_tx,M_rx),dtype=np.complex64)


    soft_power = M_tx * M_tx/D_tx
    soft_received_power = sum(sum(np.abs(P_soft)))

    print("soft_power_into_H:",soft_received_power)

    total_powers = []

    # normalized
    norm_soft = P_soft / np.linalg.norm(P_soft) # default euclidean norm


    print("normalized soft power into H:",sum(sum(np.abs(norm_soft))))
    ...
    for p in powers:
        ...
        for i in range(M_tx):
            for j in range(M_rx):
                angle = np.angle(P_soft[i][j])
                #angle = np.angle(norm_soft[i][j])
#                approx_angle = phasify(angle, p)
                approx_angle = halfify(np.pi,-np.pi,angle, p)
                #approx_amplitude = halfify(max_range, min_range, abs(P_soft[i][j]), p)
                norm_approx_amplitude = halfify(1, 0, abs(norm_soft[i][j]), p) # normalized, 0-1
                # denormalizing for capacity calcs
                approx_amplitude = norm_approx_amplitude*np.linalg.norm(P_soft)
                soft_approxes[p-1][i][j] = approx_amplitude*np.exp(approx_angle*1.0j) 
                #
                norm_soft_approxes[p-1][i][j] = norm_approx_amplitude*np.exp(approx_angle*1.0j)
                # phase only
                phase_approxes[p-1][i][j] = abs(P_soft[i][j])*np.exp(approx_angle*1.0j)
                norm_phase_approxes[p-1][i][j] = abs(norm_soft[i][j])*np.exp(approx_angle*1.0j)

#                total_power += np.abs(soft_approxes[p-1][i][j])
                total_power += approx_amplitude
        print("(now normalized) power_into_H used:",total_power,"angle:",approx_angle)
        total_powers.append(total_power)
#        soft_approxes[p-1] = soft_approxes[p-1] * soft_power/total_power
...
#    print("w prime 2 solved, w prime prime:", w_prime_2_solved,w_prime_solved)
    ...
    # normalized, no reduction, SI
    norm_SI = rel_nonzero_CSI / np.linalg.norm(rel_nonzero_CSI)
    ...
    norm_RxSums = np.sum(norm_SI,0)
    oldSI = np.sum(np.multiply(RxSums,RxSums.conjugate()))
    oldSI_norm = np.sum(np.multiply(norm_RxSums,norm_RxSums.conjugate()))
    print("SI before:\n", oldSI)
    print("normalized:\n", oldSI_norm)

    RxSums2 = np.sum(P_soft,0)
    norm_RxSums2 = np.sum(norm_soft,0)
    softSI = np.sum(np.multiply(RxSums2,RxSums2.conjugate()))
    softSI_norm = np.sum(np.multiply(norm_RxSums2,norm_RxSums2.conjugate()))
    print("SI after SoftNull with",D_tx,"effective antennas:\n",softSI)
    print("normalized:\n",softSI_norm)

    # soft approx self-interference measurements
    saSIs = np.zeros((len(powers)))
    norm_saSIs = np.zeros((len(powers)))
    phase_saSIs = np.zeros((len(powers)))

    # SI after softnull approximations
    for p in powers:
        RxSums2b = np.sum(soft_approxes[p-1],0)
        norm_RxSums2b = np.sum(norm_soft_approxes[p-1],0)
        phase_RxSums2b = np.sum(phase_approxes[p-1],0)

        saSI = np.sum(np.multiply(RxSums2b,RxSums2b.conjugate()))
        saSI_norm = np.sum(np.multiply(norm_RxSums2b,norm_RxSums2b.conjugate()))
        phase_saSI = np.sum(np.multiply(phase_RxSums2b,phase_RxSums2b.conjugate()))

        saSIs[p-1] = saSI
        norm_saSIs[p-1] = saSI_norm
        phase_saSIs[p-1] = phase_saSI

        print("SI after approximating SoftNull with",D_tx,"effective antennas and",p,"bits of granularity:\n",saSI)
        print("received power adjusted:\n", saSI * soft_received_power/total_powers[i])
        print("normalzied saSI:",saSI_norm)
        print("phase only:",phase_saSI)
    ...
    # scale client rx for each antenna by how much that channel changed after softnull
#    cl_rx_sum_soft = np.sum(np.multiply(rel_cl_rx,np.divide(P_soft, )),0)
...
#    cl_rx_SNR = np.dot(rel_SNRs_rx.reshape((1,-1)), np.ravel(X_down).reshape((-1,1)))
    ...
    cl_tx_SNR = cl_tx_sig / (cl_tx_noise + oldSI)
...
#    cl_rx_sum_soft = 
#    cl_rx_sig_soft = cl_rx_sig # assuming total received at client does not change depending on antenna weight

    # the actual correct way
    temp = np.matmul(np.asarray(rel_cl_rx).T,soft_weights)
    cl_rx_sum_soft = np.sum(np.matmul(temp,P_down),0)
    cl_rx_sig_soft = np.sum(np.multiply(cl_rx_sum_soft,cl_rx_sum_soft.conjugate()))
    '''
    cl_rx_sig_soft = 0
    for i, ant in enumerate(rel_cl_rx):
        ant_sig = ant * soft_weights[i]
        cl_rx_sig_soft += np.multiply(ant_sig,ant_sig.conjugate())
    '''
...
#    cl_rx_SNR_soft = np.dot(rel_SNRs_rx, np.ravel(P_soft))
 #   cl_rx_SNR_soft = np.dot(rel_SNRs_rx.reshape((1,-1), np.ravel(P_soft).reshape(-1,1)))
  #  cl_tx_SNR_soft = np.matmul(P_soft, rel_SNRs_tx)
    ...
    cl_tx_SNR_soft = cl_tx_sig_soft / (cl_tx_noise + softSI)
    ...
    # client signals, SNRs, capacities after softnull approximations:
    cl_rx_sigs_sa = []
    cl_rx_SNRs_sa = []
    cl_rx_caps_sa = []
    cl_tx_SNRs_sa = []
    ...
    for i in range(len(total_powers)):
        sa_sig = cl_rx_sig * soft_received_power/total_powers[i] # scale by power used in softnull vs sa
        cl_rx_sigs_sa.append(sa_sig)
        sa_SNR = sa_sig / (cl_rx_noise + 0)
        cl_rx_SNRs_sa.append(sa_SNR)
        sa_cap = bandwidth * np.log2(1+sa_SNR)
        cl_rx_caps_sa.append(sa_cap)
        
        sa_tx_SINR = cl_tx_sig / (cl_tx_noise + saSIs[i])
        cl_tx_SNRs_sa.append(sa_tx_SINR)
        sa_tx_cap = bandwidth * np.log2(1+sa_tx_SINR)
        cl_tx_caps_sa.append(sa_tx_cap)
    ...
    print("Client rx capacity after softnull")
    print(cl_rx_cap_soft)
    print("Client tx capacity after softnull")
    print(cl_tx_cap_soft)

    for i in range(8):
        print("client rx capacity after",i+1,"bit approximation:")
        print(cl_rx_caps_sa[i])
        print("tx capacity:")
        print(cl_tx_caps_sa[i])
...
#    print("last soft_approxes")
#    print(soft_approxes[-1])

    print("saSIs:\n", saSIs)
    print("sa Caps rx:\n", cl_rx_caps_sa)
    print("sa Caps tx:\n", cl_tx_caps_sa)

    print("(total capacity before, cl rx, cl tx:\n")
    print("(total capacity softnull, cl rx, cl tx,normweights:\n")
    print(np.real(cl_rx_cap), np.real(cl_tx_cap))
    print(np.real(cl_rx_cap_soft), np.real(cl_tx_cap_soft), np.linalg.norm(soft_weights))
    
    print("total capacity sa:\n")
    for (item1, item2) in zip(cl_rx_caps_sa, cl_tx_caps_sa):
        print(np.real(item1),np.real(item2))
...
######################
    '''
    # softnull approximations by granularity
    X = powers
    Y = saSIs / softSI
    Y2 = norm_saSIs / softSI_norm
    plt.plot(X,Y2)
    plt.style.use('seaborn-whitegrid')
    print("Showing: softnull approximation over softnull ratio by number of bits of resolution")
    plt.show()

    # softnull phase-only approximations by granularity
    X = powers
    Y3 = phase_saSIs / softSI
    plt.plot(X,Y3)
    plt.style.use('seaborn-whitegrid')
    print("Showing: phase-only approximations")
    plt.show()
    '''
...
################
    ...
    with open("./text_data/softnull_SI.txt", "a") as f:
        f.write(teststr+",")
        f.write(str(np.real(softSI)))
        f.write("\n")
    with open("./text_data/softnull_SI_normalized.txt", "a") as f:
        f.write(teststr+",")
        f.write(str(np.real(softSI_norm)))
        f.write("\n")

    with open("./text_data/original_SI.txt", "a") as f:
        f.write(teststr+",")
        f.write(str(np.real(oldSI)))
        f.write("\n")
    with open("./text_data/original_SI_normalized.txt", "a") as f:
        f.write(teststr+",")
        f.write(str(np.real(oldSI_norm)))
        f.write("\n")
#####
    with open("./text_data/hybrid_SI.txt", "a") as f:
        f.write(teststr+",")
        for sa in saSIs:
            f.write(",")
            f.write(str(np.real(sa)))
        f.write("\n")
    with open("./text_data/hybrid_SI_normalized.txt", "a") as f:
        f.write(teststr+",")
        for sa in norm_saSIs:
            f.write(",")
            f.write(str(np.real(sa)))
        f.write("\n")

    with open("./text_data/phase_SI.txt", "a") as f:
        f.write(teststr+",")
        for sa in phase_saSIs:
            f.write(str(np.real(sa)))
            f.write(",")
        f.write("\n")


    with open("./text_data/original_capacity.txt", "a") as f:
        f.write(teststr+",")
        f.write(str(np.real(cl_rx_cap + cl_tx_cap)))
        f.write("\n")

    with open("./text_data/softnull_capacity.txt", "a") as f:
        f.write(teststr+",")
        f.write(str(np.real(cl_rx_cap_soft + cl_tx_cap_soft)))
        f.write("\n")

    with open("./text_data/hybrid_capacity.txt", "a") as f:
        f.write(teststr+",")
        for (cap1,cap2) in zip(cl_tx_caps_sa,cl_rx_caps_sa):
            f.write(",")
            f.write(str(np.real(cap1+cap2)))
        f.write("\n")
...
#    parser.add_option("--sample-cal", action="store_true", dest="samp_cal", help="perform sample offset calibration",default=False)
    ...
    parser.add_option("--effective", type="int", dest="effective", help="Number of effective antennas to reduce down to for Softnull (D_tx)", default=3)
